...
func (i *Instance) GetMiddleware(next http.Handler) http.Handler {
...
func (i *Instance) exposeHeadersParse(writer http.ResponseWriter) {
	var exposeHeaders = i.config.ExposeHeaders
	if exposeHeaders == nil {
		return
	}

	var exposedHeaders = strings.Join(exposeHeaders, ", ")
	writer.Header().Add("Access-Control-Expose-Headers", exposedHeaders)
}
